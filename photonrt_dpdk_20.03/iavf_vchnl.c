...
Sep 2021 - Added option to propogate return code when PF rejects
OpCode.
         -  Each  Handler gracefully handles respected action.

         -  It probably better to hold a bit vector in aivf info struct
            so upper layer can detect better specific issue with OpCode. 

        -   TODO Test with different TX/RX with default and not default queues len.
	...
	default:
		...
		do {
			if (vf->pend_cmd == VIRTCHNL_OP_UNKNOWN) {
	                        PMD_DRV_LOG(ERR, "Returned  VIRTCHNL_OP_UNKNOWN");
				break;
			}
			rte_delay_ms(ASQ_DELAY_MS);
			/* If don't read msg or read sys event, continue */
		...
		if (i >= MAX_TRY_TIMES) {
			...
			if (vf->cmd_retval == 0) {
			    PMD_DRV_LOG(ERR,
					"Unsported cmd %d return val %d", args->ops, vf->cmd_retval);
			    err = -ENOTSUP;
                        } else {
			    err = -EIO;
			}
...
iavf_enable_vlan_strip(struct iavf_adapter *adapter)
{
	...
	err = iavf_execute_vf_cmd(adapter, &args);
        ...
        if (err == -ENOTSUP) {
             PMD_DRV_LOG(ERR, "Unsuported OP_ENABLE_VLAN_STRIPPING");
             return 0;
        }
...
iavf_disable_vlan_strip(struct iavf_adapter *adapter)
{
	...
	ret = iavf_execute_vf_cmd(adapter, &args);
        ...
        if (ret == -ENOTSUP) {
		PMD_DRV_LOG(ERR, "Unsuported OP_DISABLE_VLAN_STRIPPING");
		return 0;
	}
...
iavf_get_vf_resource(struct iavf_adapter *adapter)
{
	...
	err = iavf_execute_vf_cmd(adapter, &args);
        if (err == -ENOTSUP) {
                PMD_DRV_LOG(ERR, "Unsuported OP_GET_VF_RESOURCE");
                return 0;
        }
...
iavf_enable_queues(struct iavf_adapter *adapter)
{
	...
	if (err == -ENOTSUP) {
                PMD_DRV_LOG(ERR, "Unsuported OP_ENABLED_QUEUES");
		return 0;
	}
		 ...
		 bool rx, bool on)
{
    ...
    if (err == -ENOTSUP) {
        PMD_DRV_LOG(ERR, "Unsuported OP_ENABLED_QUEUES | OP_DISABLE_QUEUES");
        return 0;
    }
...
iavf_configure_rss_lut(struct iavf_adapter *adapter)
{
	...
	if (err == -ENOTSUP) {
		PMD_DRV_LOG(ERR, "Unsuported OP_CONFIG_RSS_LUT");
		err = 0;
	}
		...
		uint16_t num_queue_pairs, uint16_t index)
{
	...
	if (err == -ENOTSUP) {
             PMD_DRV_LOG(ERR, "Unsuported VIRTCHNL_OP_CONFIG_VSI_QUEUES");
             err = 0;
        }
...
iavf_config_irq_map(struct iavf_adapter *adapter)
{
	...
	err = iavf_execute_vf_cmd(adapter, &args);
	if (err == -ENOTSUP) {
             PMD_DRV_LOG(ERR, "Unsuported OP_CONFIG_IRQ_MAP");
             return 0;
        }
		...
		struct virtchnl_eth_stats **pstats)
{
	...
	err = iavf_execute_vf_cmd(adapter, &args);
        if (err == -ENOTSUP) {
             PMD_DRV_LOG(ERR, "Unsuported OP_GET_STATS");
	     *pstats = NULL;
             err = 0;
        }
		     ...
		     struct virtchnl_rss_cfg *rss_cfg, bool add)
{
    ...
    if (err == -ENOTSUP) {
        PMD_DRV_LOG(ERR, "Unsuported OP_ADD_RSS_CFG OP_DEL_RSS_INPUT_CFG");
        return 0;
    }
			...
			uint32_t mc_addrs_num, bool add)
{
    ...
    if (err == -ENOTSUP) {
        PMD_DRV_LOG(ERR, "Unsuported OP_ENABLED_QUEUES OP_DEL_ETH_ADDR");
        return 0;
    }
...
iavf_get_max_rss_queue_region(struct iavf_adapter *adapter)
{
	...
	err = iavf_execute_vf_cmd(adapter, &args);
    if (err == -ENOTSUP) {
        PMD_DRV_LOG(ERR, "Unsuported VIRTCHNL_OP_GET_MAX_RSS_QREGION");
        return 0;
     }
	...
	qregion_width = ((struct virtchnl_max_rss_qregion *)args.out_buffer)->qregion_width;
